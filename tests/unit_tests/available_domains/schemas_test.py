...
def test_dump_with_domains() -> None:
...
def test_dump_with_empty_domains() -> None:
    schema = AvailableDomainsSchema()
    result = schema.dump({"domains": []})
    assert result == {"domains": []}


def test_dump_with_none_domains() -> None:
    schema = AvailableDomainsSchema()
    result = schema.dump({"domains": None})
    assert result == {"domains": None}


def test_load_with_domains() -> None:
    schema = AvailableDomainsSchema()
    result = schema.load({"domains": ["https://example.com"]})
    assert result["domains"] == ["https://example.com"]


def test_load_with_empty_input() -> None:
    schema = AvailableDomainsSchema()
    result = schema.load({})
    assert "domains" not in result or result.get("domains") is None


def test_dump_with_single_domain() -> None:
    schema = AvailableDomainsSchema()
    result = schema.dump({"domains": ["https://only.example.com"]})
    assert result == {"domains": ["https://only.example.com"]}
