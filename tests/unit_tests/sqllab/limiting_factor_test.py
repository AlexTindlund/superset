...
def test_limiting_factor_is_string() -> None:
...
def test_limiting_factor_membership() -> None:
    values = {member.value for member in LimitingFactor}
    assert "QUERY" in values
    assert "DROPDOWN" in values
    assert "QUERY_AND_DROPDOWN" in values
    assert "NOT_LIMITED" in values
