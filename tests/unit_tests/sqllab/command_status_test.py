...
def test_execution_status_member_count() -> None:
    assert len(SqlJsonExecutionStatus) == 4


def test_execution_status_by_value() -> None:
...
def test_execution_status_by_name() -> None:
    assert (
        SqlJsonExecutionStatus["QUERY_ALREADY_CREATED"]
        is SqlJsonExecutionStatus.QUERY_ALREADY_CREATED
    )
    assert SqlJsonExecutionStatus["FAILED"] is SqlJsonExecutionStatus.FAILED


def test_execution_status_names() -> None:
    names = {member.name for member in SqlJsonExecutionStatus}
    assert names == {
        "QUERY_ALREADY_CREATED",
        "HAS_RESULTS",
        "QUERY_IS_RUNNING",
        "FAILED",
    }
